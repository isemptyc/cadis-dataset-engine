...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "south_korea"

KR_PROFILE = AdminProfile(
    name_keys=("name:ko", "name", "official_name", "name:en"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("ko", "en", "zh"),
)


class SouthKoreaAdminEngine(DatasetBuildEngineBase):
    ENGINE = "kr_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "south_korea_admin.json"
        self._ffsf_dataset_path = self._work_dir / "south_korea_admin.bin"
        self._ffsf_meta_path = self._work_dir / "KR_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "south_korea_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "SouthKoreaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._admin_hierarchy_path.exists():
            ...
            if not nodes_path.exists() or not edges_path.exists():
                extract_admin_hierarchy(
                    ...
                    name_keys=KR_PROFILE.name_keys,
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="kr-admin-semantic-1.0.0",
