...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "montenegro"

ME_PROFILE = AdminProfile(
    name_keys=("name:sr-Latn", "name:sr", "name", "name:en", "official_name"),
    level_policies={
        6: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("cnr-latn", "en", "ru", "sr", "sr-latn"),
)
...
class MontenegroAdminEngine(GermanyAdminEngine):
    ENGINE = "me_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [6, 8, 9]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((6, 8, 9))
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "montenegro_admin.json"
        self._ffsf_dataset_path = self._work_dir / "montenegro_admin.bin"
        self._ffsf_meta_path = self._work_dir / "ME_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "montenegro_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "MontenegroAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="me-admin-semantic-1.0.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "allowed_levels": [6, 8, 9],
            "allowed_shapes": allowed_shapes,
            "shape_status": [
                {
                    "levels": shape,
                    "status": "ok" if 6 in shape else "partial",
                }
