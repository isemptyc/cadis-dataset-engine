...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "vietnam"

VN_PROFILE = AdminProfile(
    name_keys=("name:vi", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("en", "vi", "zh", "zh-hans", "zh-hant"),
)


class VietnamAdminEngine(GermanyAdminEngine):
    ENGINE = "vn_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "vietnam_admin.json"
        self._ffsf_dataset_path = self._work_dir / "vietnam_admin.bin"
        self._ffsf_meta_path = self._work_dir / "VN_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "vietnam_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "VietnamAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        if not self._admin_dataset_path.exists():
            build_admin_dataset(
                ...
                level_labels={
                    4: "admin_province_municipality",
                    6: "admin_commune_ward_special_zone",
                },
                id_prefix="vn",
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="vn-admin-semantic-1.0.0",
