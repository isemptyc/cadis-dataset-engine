...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "finland"

FI_PROFILE = AdminProfile(
    name_keys=("name:fi", "name", "name:sv", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("fi", "sv", "en"),
)


class FinlandAdminEngine(DatasetBuildEngineBase):
    ENGINE = "fi_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "finland_admin.json"
        self._ffsf_dataset_path = self._work_dir / "finland_admin.bin"
        self._ffsf_meta_path = self._work_dir / "FI_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "finland_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "FinlandAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="fi-admin-semantic-1.0.0",
