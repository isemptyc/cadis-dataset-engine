...
AU_ALLOWED_SHAPES = {
    ...
    for shape in combinations(AU_LEVELS, size)
}

AU_PROFILE = AdminProfile(
...
class AustraliaAdminEngine(DatasetBuildEngineBase):
    ENGINE = "au_admin"
    VERSION = "v0.1"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = AU_LEVELS
    ALLOWED_SHAPES = AU_ALLOWED_SHAPES
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "australia_admin.json"
        self._ffsf_dataset_path = self._work_dir / "australia_admin.bin"
        self._ffsf_meta_path = self._work_dir / "AU_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "australia_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "AustraliaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="au-admin-semantic-0.1.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "shape_status": [
                {
                    "levels": list(shape),
                    "status": "ok" if shape in ok_shapes else "partial",
                }
