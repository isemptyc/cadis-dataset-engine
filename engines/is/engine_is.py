...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "iceland"

IS_PROFILE = AdminProfile(
    name_keys=("name:is", "name", "name:en", "official_name"),
    level_policies={
        5: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("is", "en"),
)


class IcelandAdminEngine(DatasetBuildEngineBase):
    ENGINE = "is_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "iceland_admin.json"
        self._ffsf_dataset_path = self._work_dir / "iceland_admin.bin"
        self._ffsf_meta_path = self._work_dir / "IS_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "iceland_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "IcelandAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="is-admin-semantic-1.0.0",
