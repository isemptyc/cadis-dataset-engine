...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "macedonia"

MK_PROFILE = AdminProfile(
    name_keys=("name:mk", "name:sq", "name", "name:en", "official_name"),
    level_policies={
        7: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("en", "mk", "sq", "sr", "tr"),
)
...
class MacedoniaAdminEngine(GermanyAdminEngine):
    ENGINE = "mk_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [7, 8, 9]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((7, 8, 9))
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "macedonia_admin.json"
        self._ffsf_dataset_path = self._work_dir / "macedonia_admin.bin"
        self._ffsf_meta_path = self._work_dir / "MK_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "macedonia_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "MacedoniaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="mk-admin-semantic-1.0.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "allowed_levels": [7, 8, 9],
            "allowed_shapes": allowed_shapes,
            "shape_status": [
                {
                    "levels": shape,
                    "status": "ok" if 7 in shape else "partial",
                }
