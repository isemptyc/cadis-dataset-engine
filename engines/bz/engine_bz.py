...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "belize"

BZ_PROFILE = AdminProfile(
...
class BelizeAdminEngine(BrazilAdminEngine):
    ENGINE = "bz_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [4, 7, 8, 9, 10, 11]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((4, 7, 8, 9, 10, 11))
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "belize_admin.json"
        self._ffsf_dataset_path = self._work_dir / "belize_admin.bin"
        self._ffsf_meta_path = self._work_dir / "BZ_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "belize_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "BelizeAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="bz-admin-semantic-1.0.0",
