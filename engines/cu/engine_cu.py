...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "cuba"

CU_PROFILE = AdminProfile(
...
class CubaAdminEngine(BrazilAdminEngine):
    ENGINE = "cu_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "cuba_admin.json"
        self._ffsf_dataset_path = self._work_dir / "cuba_admin.bin"
        self._ffsf_meta_path = self._work_dir / "CU_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "cuba_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError("CubaAdminEngine in cadis-dataset-engine is build-only. Provide osm_pbf_path.")
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            export_admin_semantic_dataset(
                ...
                version="cu-admin-semantic-1.0.0",
