...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "suriname"

SR_PROFILE = AdminProfile(
    name_keys=("name:nl", "name", "official_name", "name:en"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("nl", "en"),
)
...
class SurinameAdminEngine(BrazilAdminEngine):
    ENGINE = "sr_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [4, 6]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((4, 6))
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "suriname_admin.json"
        self._ffsf_dataset_path = self._work_dir / "suriname_admin.bin"
        self._ffsf_meta_path = self._work_dir / "SR_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "suriname_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "SurinameAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="sr-admin-semantic-1.0.0",
