...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "peru"

PE_PROFILE = AdminProfile(
...
class PeruAdminEngine(BrazilAdminEngine):
    ENGINE = "pe_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "peru_admin.json"
        self._ffsf_dataset_path = self._work_dir / "peru_admin.bin"
        self._ffsf_meta_path = self._work_dir / "PE_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "peru_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "PeruAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="pe-admin-semantic-1.0.0",
