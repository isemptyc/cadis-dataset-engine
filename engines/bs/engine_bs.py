...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "bahamas"

BS_PROFILE = AdminProfile(
...
class BahamasAdminEngine(BrazilAdminEngine):
    ENGINE = "bs_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [8, 9]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((8, 9))
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "bahamas_admin.json"
        self._ffsf_dataset_path = self._work_dir / "bahamas_admin.bin"
        self._ffsf_meta_path = self._work_dir / "BS_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "bahamas_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "BahamasAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="bs-admin-semantic-1.0.0",
