...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "russia"

RU_PROFILE = AdminProfile(
    name_keys=("name:ru", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("en", "ja", "ru", "uk"),
)
...
class RussiaAdminEngine(GermanyAdminEngine):
    ENGINE = "ru_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "russia_admin.json"
        self._ffsf_dataset_path = self._work_dir / "russia_admin.bin"
        self._ffsf_meta_path = self._work_dir / "RU_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "russia_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "RussiaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        if not self._admin_dataset_path.exists():
            build_admin_dataset(
                ...
                level_labels={
                    4: "admin_federal_subject",
                    5: "admin_federal_city_district",
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="ru-admin-semantic-1.0.0",
