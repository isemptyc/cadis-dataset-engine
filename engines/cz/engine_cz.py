...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "czech_republic"

CZ_PROFILE = AdminProfile(
    name_keys=("name:cs", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("cs", "de"),
)


class CzechRepublicAdminEngine(GermanyAdminEngine):
    ENGINE = "cz_admin"
    VERSION = "v1.0"
    ...
    EXCLUDED_FEATURE_IDS = {
        "cz_r6076233",  # Wolanow (Poland)
        "cz_r6418836",  # Wrzosowka (Poland)
        "cz_r6419556",  # Bielice (Poland)
    }
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "czech_republic_admin.json"
        self._ffsf_dataset_path = self._work_dir / "czech_republic_admin.bin"
        self._ffsf_meta_path = self._work_dir / "CZ_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "czech_republic_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "CzechRepublicAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="cz-admin-semantic-1.0.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        return {
            ...
            "shape_status": [
                ...
                {"levels": [6, 7], "status": "partial"},
                {"levels": [6, 7, 8], "status": "partial"},
