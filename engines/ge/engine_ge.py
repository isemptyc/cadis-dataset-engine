...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "georgia"

GE_PROFILE = AdminProfile(
    name_keys=("name:ka", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("ab", "en", "ka", "os", "ru"),
)
...
class GeorgiaAdminEngine(GermanyAdminEngine):
    ENGINE = "ge_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [4, 6, 8, 10]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((4, 6, 8, 10))
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "georgia_admin.json"
        self._ffsf_dataset_path = self._work_dir / "georgia_admin.bin"
        self._ffsf_meta_path = self._work_dir / "GE_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "georgia_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "GeorgiaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="ge-admin-semantic-1.0.0",
