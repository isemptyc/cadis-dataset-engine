...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "bulgaria"

BG_PROFILE = AdminProfile(
    name_keys=("name:bg", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("bg", "en"),
)
...
class BulgariaAdminEngine(GermanyAdminEngine):
    ENGINE = "bg_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "bulgaria_admin.json"
        self._ffsf_dataset_path = self._work_dir / "bulgaria_admin.bin"
        self._ffsf_meta_path = self._work_dir / "BG_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "bulgaria_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "BulgariaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="bg-admin-semantic-1.0.0",
