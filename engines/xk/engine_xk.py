...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "kosovo"

XK_PROFILE = AdminProfile(
    name_keys=("name:sq", "name:sr", "name", "name:en", "official_name"),
    level_policies={
        5: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("en", "sq", "sr", "sr-latn"),
)
...
class KosovoAdminEngine(GermanyAdminEngine):
    ENGINE = "xk_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [5, 6, 8, 9, 10]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((5, 6, 8, 9, 10))
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "kosovo_admin.json"
        self._ffsf_dataset_path = self._work_dir / "kosovo_admin.bin"
        self._ffsf_meta_path = self._work_dir / "XK_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "kosovo_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "KosovoAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="xk-admin-semantic-1.0.0",
