...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "thailand"

TH_PROFILE = AdminProfile(
    name_keys=("name:th", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("th", "en"),
)


class ThailandAdminEngine(GermanyAdminEngine):
    ENGINE = "th_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "thailand_admin.json"
        self._ffsf_dataset_path = self._work_dir / "thailand_admin.bin"
        self._ffsf_meta_path = self._work_dir / "TH_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "thailand_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "ThailandAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="th-admin-semantic-1.0.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        return {
            ...
            "shape_status": [
                ...
                {"levels": [4, 6, 8, 10], "status": "ok"},
                {"levels": [4, 6, 10], "status": "ok"},
                {"levels": [4, 8], "status": "ok"},
                {"levels": [4, 8, 10], "status": "ok"},
                {"levels": [4, 10], "status": "ok"},
                {"levels": [6], "status": "partial"},
                {"levels": [6, 8], "status": "partial"},
                {"levels": [6, 8, 10], "status": "partial"},
                {"levels": [6, 10], "status": "partial"},
                {"levels": [8], "status": "partial"},
                {"levels": [8, 10], "status": "partial"},
                {"levels": [10], "status": "partial"},
            ],
            "layers": {
