...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "bosnia_herzegovina"

BA_PROFILE = AdminProfile(
    name_keys=("name:bs", "name:sr", "name:hr", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("bs", "sr", "hr", "sr-latn", "en"),
)
...
class BosniaHerzegovinaAdminEngine(GermanyAdminEngine):
    ENGINE = "ba_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "bosnia_herzegovina_admin.json"
        self._ffsf_dataset_path = self._work_dir / "bosnia_herzegovina_admin.bin"
        self._ffsf_meta_path = self._work_dir / "BA_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "bosnia_herzegovina_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "BosniaHerzegovinaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        if not self._admin_dataset_path.exists():
            build_admin_dataset(
                ...
                level_labels={
                    4: "admin_entity_or_district",
                    5: "admin_canton",
                    6: "admin_city",
                    7: "admin_municipality",
                    8: "admin_local_community",
                },
                id_prefix="ba",
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="ba-admin-semantic-1.0.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "shape_status": [
                {
                    "levels": shape,
                    "status": "ok" if 4 in shape else "partial",
                }
