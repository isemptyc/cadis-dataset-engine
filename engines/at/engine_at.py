...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "austria"

AT_PROFILE = AdminProfile(
    name_keys=("name", "name:de", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("de", "hu", "hr", "sl", "en"),
)


class AustriaAdminEngine(GermanyAdminEngine):
    ENGINE = "at_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "austria_admin.json"
        self._ffsf_dataset_path = self._work_dir / "austria_admin.bin"
        self._ffsf_meta_path = self._work_dir / "AT_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "austria_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "AustriaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="at-admin-semantic-1.0.0",
