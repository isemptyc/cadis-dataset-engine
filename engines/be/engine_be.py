...
class BelgiumAdminEngine(DatasetBuildEngineBase):
    ...
    EXCLUDED_FEATURE_IDS = {
        "be_r2971481",  # Écouviez (France)
        "be_r3264314",  # Williers (France)
        "be_r414436",   # Les Hautes-Rivières (France)
        "be_r55171",    # Bousignies-sur-Roc (France)
    }
    PARENT_OVERRIDES = {
        "be_r58248": "be_r54094",  # Saint-Gilles -> Bruxelles-Capitale
        "be_r58249": "be_r54094",  # Forest
        "be_r58250": "be_r54094",  # Ixelles
        "be_r58252": "be_r54094",  # Etterbeek
        "be_r58253": "be_r54094",  # Uccle
        "be_r58254": "be_r54094",  # Anderlecht
        "be_r58255": "be_r54094",  # Molenbeek-Saint-Jean
        "be_r58256": "be_r54094",  # Koekelberg
        "be_r58257": "be_r54094",  # Ganshoren
        "be_r58258": "be_r54094",  # Jette
        "be_r58260": "be_r54094",  # Schaerbeek
        "be_r58262": "be_r54094",  # Saint-Josse-ten-Noode
        "be_r58263": "be_r54094",  # Auderghem
        "be_r58264": "be_r54094",  # Watermael-Boitsfort
        "be_r58274": "be_r54094",  # Bruxelles
        "be_r60140": "be_r54094",  # Berchem-Sainte-Agathe
        "be_r60144": "be_r54094",  # Evere
        "be_r60167": "be_r54094",  # Woluwe-Saint-Lambert
        "be_r60168": "be_r54094",  # Woluwe-Saint-Pierre
    }
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "belgium_admin.json"
        self._ffsf_dataset_path = self._work_dir / "belgium_admin.bin"
        self._ffsf_meta_path = self._work_dir / "BE_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "belgium_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "BelgiumAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="be-admin-semantic-1.0.0",
    ...
    def _apply_dataset_overrides(self) -> None:
        if not self._admin_dataset_path.exists():
            return
        ...
        def _rewrite_rows(rows: list) -> list:
            nonlocal changed
            rewritten: list = []
            for row in rows:
                ...
                if feature_id in self.EXCLUDED_FEATURE_IDS:
                    changed = True
                    continue

                override_parent = self.PARENT_OVERRIDES.get(feature_id)
                if override_parent is not None and row.get("parent") != override_parent:
                    row = dict(row)
                    row["parent"] = override_parent
                    changed = True
        ...
        if isinstance(admin_by_level, dict):
            for key, value in list(admin_by_level.items()):
                if isinstance(value, list):
                    admin_by_level[key] = _rewrite_rows(value)

        for key, value in list(payload.items()):
            if isinstance(value, list):
                payload[key] = _rewrite_rows(value)
