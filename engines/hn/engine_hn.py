...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "honduras"

HN_PROFILE = AdminProfile(
...
class HondurasAdminEngine(BrazilAdminEngine):
    ENGINE = "hn_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [4, 6, 7, 8, 9, 10, 12]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((4, 6, 7, 8, 9, 10, 12))
    ...
    def __init__(self, *, osm_pbf_path: str | Path | None = None, work_dir: Path | None = None, country_geometry_path: str | Path | None = None):
        ...
        self._admin_dataset_path = self._work_dir / "honduras_admin.json"
        self._ffsf_dataset_path = self._work_dir / "honduras_admin.bin"
        self._ffsf_meta_path = self._work_dir / "HN_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "honduras_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError("HondurasAdminEngine in cadis-dataset-engine is build-only. Provide osm_pbf_path.")
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            export_admin_semantic_dataset(nodes=self._build_semantic_nodes(), output_path=self._semantic_dataset_path, version="hn-admin-semantic-1.0.0", country=self.COUNTRY_ISO, source="admin_tree.txt")
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "allowed_levels": [4, 6, 7, 8, 9, 10, 12],
            ...
            "hierarchy_repair_rules": {"parent_level": 4, "child_levels": [6, 7, 8, 9, 10, 12]},
