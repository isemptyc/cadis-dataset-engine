...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "greece"
...
GR_PROFILE = AdminProfile(
    ...
    multilingual_allowed_languages=("el", "en", "fr", "de"),
)


class GreeceAdminEngine(DatasetBuildEngineBase):
    ENGINE = "gr_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "greece_admin.json"
        self._ffsf_dataset_path = self._work_dir / "greece_admin.bin"
        self._ffsf_meta_path = self._work_dir / "GR_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "greece_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "GreeceAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="gr-admin-semantic-1.0.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        return {
            ...
            "shape_status": [
                ...
                {"levels": [4, 5, 6, 7], "status": "ok"},
                {"levels": [4, 5, 6, 7, 8], "status": "ok"},
                {"levels": [4, 5, 6, 8], "status": "ok"},
                {"levels": [4, 5, 7], "status": "ok"},
                {"levels": [4, 5, 7, 8], "status": "ok"},
                {"levels": [4, 5, 8], "status": "partial"},
                {"levels": [4, 6], "status": "ok"},
                {"levels": [4, 6, 7], "status": "ok"},
                {"levels": [4, 6, 7, 8], "status": "ok"},
                {"levels": [4, 6, 8], "status": "ok"},
                {"levels": [4, 7], "status": "ok"},
                {"levels": [4, 7, 8], "status": "ok"},
                {"levels": [4, 8], "status": "partial"},
                {"levels": [5], "status": "partial"},
                {"levels": [5, 6], "status": "ok"},
                {"levels": [5, 6, 7], "status": "ok"},
                {"levels": [5, 6, 7, 8], "status": "ok"},
                {"levels": [5, 6, 8], "status": "ok"},
                {"levels": [5, 7], "status": "ok"},
                {"levels": [5, 7, 8], "status": "ok"},
