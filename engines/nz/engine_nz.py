...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "new_zealand"

NZ_PROFILE = AdminProfile(
    name_keys=("name:en", "name", "name:mi", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("en", "mi"),
)


class NewZealandAdminEngine(DatasetBuildEngineBase):
    ENGINE = "nz_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "new_zealand_admin.json"
        self._ffsf_dataset_path = self._work_dir / "new_zealand_admin.bin"
        self._ffsf_meta_path = self._work_dir / "NZ_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "new_zealand_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "NewZealandAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        if not self._admin_dataset_path.exists():
            build_admin_dataset(
                ...
                level_labels={
                    4: "admin_region",
                    6: "admin_territorial_authority",
                    8: "admin_local_board_community",
                },
                id_prefix="nz",
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="nz-admin-semantic-1.0.0",
