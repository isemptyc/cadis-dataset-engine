...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "albania"

AL_PROFILE = AdminProfile(
    name_keys=("name:sq", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("mk", "el", "en"),
)


class AlbaniaAdminEngine(GermanyAdminEngine):
    ENGINE = "al_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "albania_admin.json"
        self._ffsf_dataset_path = self._work_dir / "albania_admin.bin"
        self._ffsf_meta_path = self._work_dir / "AL_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "albania_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "AlbaniaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        if not self._admin_dataset_path.exists():
            build_admin_dataset(
                ...
                level_labels={
                    4: "admin_statistical_region",
                    6: "admin_county",
                    7: "admin_municipality",
                    8: "admin_administrative_unit",
                },
                id_prefix="al",
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="al-admin-semantic-1.0.0",
