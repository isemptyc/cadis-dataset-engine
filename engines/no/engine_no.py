...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "norway"

NO_PROFILE = AdminProfile(
    name_keys=("name:no", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("nb", "nn", "en"),
)


class NorwayAdminEngine(DatasetBuildEngineBase):
    ENGINE = "no_admin"
    VERSION = "v1.0"
    ...
    EXCLUDED_FEATURE_IDS = {
        "no_r935657",  # Eda kommun (Sweden) leaks into the Norway extract near the border.
    }
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "norway_admin.json"
        self._ffsf_dataset_path = self._work_dir / "norway_admin.bin"
        self._ffsf_meta_path = self._work_dir / "NO_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "norway_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "NorwayAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="no-admin-semantic-1.0.0",
