...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "latvia"

LV_PROFILE = AdminProfile(
    name_keys=("name:lv", "name", "name:en", "official_name"),
    level_policies={
        5: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("be", "lt", "lv", "pl", "ru"),
)
...
class LatviaAdminEngine(GermanyAdminEngine):
    ENGINE = "lv_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [5, 7, 8, 9]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((5, 7, 8, 9))
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "latvia_admin.json"
        self._ffsf_dataset_path = self._work_dir / "latvia_admin.bin"
        self._ffsf_meta_path = self._work_dir / "LV_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "latvia_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "LatviaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="lv-admin-semantic-1.0.0",
