...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "ireland"


IE_PROFILE = AdminProfile(
    ...
    multilingual_allowed_languages=("en", "ga"),
)
...
class IrelandAdminEngine(DenmarkAdminEngine):
    ENGINE = "ie_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [5, 6, 7, 8, 9]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((5, 6, 7, 8, 9))
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "ireland_admin.json"
        self._ffsf_dataset_path = self._work_dir / "ireland_admin.bin"
        self._ffsf_meta_path = self._work_dir / "IE_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "ireland_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "IrelandAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        if not self._admin_dataset_path.exists():
            build_admin_dataset(
                ...
                level_labels={
                    ...
                    6: "admin_county",
                    7: "admin_city",
                    8: "admin_municipal_district",
                    9: "admin_civil_parish",
                },
                id_prefix="ie",
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="ie-admin-semantic-1.0.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        payload = super(DenmarkAdminEngine, self)._runtime_policy_payload()
        if payload is None:
            payload = {}
        payload["nearby_policy"] = {
            "enabled": True,
            "max_distance_km": 2.0,
