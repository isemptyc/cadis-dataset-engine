...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "switzerland"

CH_PROFILE = AdminProfile(
    name_keys=("name", "name:de", "name:fr", "name:it", "name:rm", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("de", "fr", "it", "rm", "en"),
)


class SwitzerlandAdminEngine(DatasetBuildEngineBase):
    ENGINE = "ch_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "switzerland_admin.json"
        self._ffsf_dataset_path = self._work_dir / "switzerland_admin.bin"
        self._ffsf_meta_path = self._work_dir / "CH_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "switzerland_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "SwitzerlandAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="ch-admin-semantic-1.0.0",
    ...
    def _write_dataset_scoped_hierarchy_artifacts(self) -> None:
        ...
        for node in dataset_nodes:
            ...
            hierarchy_nodes.append(
                {
                    "id": raw_id,
                    "osm_id": raw_id[3:] if raw_id.startswith("ch_") else raw_id,
                    "name": node["name"],
