...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "bhutan"

BT_PROFILE = AdminProfile(
    name_keys=("name:en", "name", "official_name", "name:bt", "name:ru"),
    level_policies={
    ...
    multilingual_allowed_languages=('en', 'bt', 'ru'),
)
...
class BhutanAdminEngine(BrazilAdminEngine):
    ENGINE = "bt_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [4, 5, 6, 8, 9, 10]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((4, 5, 6, 8, 9, 10,))
    ...
    def __init__(self, *, osm_pbf_path: str | Path | None = None, work_dir: Path | None = None, country_geometry_path: str | Path | None = None):
        ...
        self._admin_dataset_path = self._work_dir / "bhutan_admin.json"
        self._ffsf_dataset_path = self._work_dir / "bhutan_admin.bin"
        self._ffsf_meta_path = self._work_dir / "BT_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "bhutan_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError("BhutanAdminEngine in cadis-dataset-engine is build-only. Provide osm_pbf_path.")
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            export_admin_semantic_dataset(nodes=self._build_semantic_nodes(), output_path=self._semantic_dataset_path, version="bt-admin-semantic-1.0.0", country=self.COUNTRY_ISO, source="admin_tree.txt")
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "hierarchy_repair_rules": {"parent_level": 4, "child_levels": [level for level in [4, 5, 6, 8, 9, 10] if level != 4]},
