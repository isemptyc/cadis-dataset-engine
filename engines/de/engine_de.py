...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "germany"

DE_PROFILE = AdminProfile(
    name_keys=("name:de", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("de", "en", "fr"),
)


class GermanyAdminEngine(DatasetBuildEngineBase):
    ENGINE = "de_admin"
    VERSION = "v1.0"
    ...
    EXCLUDED_FEATURE_IDS = {
        "de_r115152",   # Erchingen (France)
        "de_r122813",   # Rolbingen (France)
        "de_r122902",   # Schöneck (France)
        "de_r1337084",  # Vianden (Luxembourg)
        "de_r1683623",  # Bettingen (Switzerland)
        "de_r1683696",  # Riehen (Switzerland)
        "de_r404493",   # Schärding (Austria)
        "de_r74826",    # Sibratsgfäll (Austria)
        "de_r89061",    # Niedersteinbach (France)
        "de_r89516",    # Wingen (France)
        "de_r90813",    # Bliesgersweiler (France)
        "de_r932085",   # Eichwald / Chalampé (France)
    }
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "germany_admin.json"
        self._ffsf_dataset_path = self._work_dir / "germany_admin.bin"
        self._ffsf_meta_path = self._work_dir / "DE_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "germany_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "GermanyAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="de-admin-semantic-1.0.0",
    ...
    def _apply_dataset_overrides(self) -> None:
        if not self._admin_dataset_path.exists() or not self.EXCLUDED_FEATURE_IDS:
            return
        ...
        def _rewrite_rows(rows: list) -> list:
            ...
            for row in rows:
                if not isinstance(row, dict):
                    ...
                    continue
                if row.get("id") in self.EXCLUDED_FEATURE_IDS:
                    changed = True
                    continue
                rewritten.append(row)
            return rewritten
