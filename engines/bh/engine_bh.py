...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "bahrain"

BH_PROFILE = AdminProfile(
    name_keys=("name:en", "name", "official_name", "name:bh", "name:ru"),
    level_policies={
    ...
    multilingual_allowed_languages=('en', 'bh', 'ru'),
)
...
class BahrainAdminEngine(BrazilAdminEngine):
    ENGINE = "bh_admin"
    VERSION = "v1.0"
    ...
    def __init__(self, *, osm_pbf_path: str | Path | None = None, work_dir: Path | None = None, country_geometry_path: str | Path | None = None):
        ...
        self._admin_dataset_path = self._work_dir / "bahrain_admin.json"
        self._ffsf_dataset_path = self._work_dir / "bahrain_admin.bin"
        self._ffsf_meta_path = self._work_dir / "BH_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "bahrain_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError("BahrainAdminEngine in cadis-dataset-engine is build-only. Provide osm_pbf_path.")
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            export_admin_semantic_dataset(nodes=self._build_semantic_nodes(), output_path=self._semantic_dataset_path, version="bh-admin-semantic-1.0.0", country=self.COUNTRY_ISO, source="admin_tree.txt")
