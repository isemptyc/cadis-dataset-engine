"""Country dataset engines for cadis-dataset-engine."""

from .ar import ArgentinaAdminEngine
from .au import AustraliaAdminEngine
from .be import BelgiumAdminEngine
from .br import BrazilAdminEngine
from .ca import CanadaAdminEngine
from .cz import CzechRepublicAdminEngine
from .dk import DenmarkAdminEngine
from .de import GermanyAdminEngine
from .fr import FranceAdminEngine
from .fi import FinlandAdminEngine
from .gb import GreatBritainAdminEngine
from .it import ItalyAdminEngine
from .jp import JapanAdminEngine
from .kr import SouthKoreaAdminEngine
from .my import MalaysiaAdminEngine
from .mx import MexicoAdminEngine
from .nl import NetherlandsAdminEngine
from .no import NorwayAdminEngine
from .nz import NewZealandAdminEngine
from .ph import PhilippinesAdminEngine
from .pt import PortugalAdminEngine
from .se import SwedenAdminEngine
from .sg import SingaporeAdminEngine
from .ch import SwitzerlandAdminEngine
from .th import ThailandAdminEngine
from .tw import TaiwanAdminEngine

__all__ = [
    "ArgentinaAdminEngine",
    "AustraliaAdminEngine",
    "BelgiumAdminEngine",
    "BrazilAdminEngine",
    "CanadaAdminEngine",
    "CzechRepublicAdminEngine",
    "DenmarkAdminEngine",
    "GermanyAdminEngine",
    "FranceAdminEngine",
    "FinlandAdminEngine",
    "GreatBritainAdminEngine",
    "ItalyAdminEngine",
    "JapanAdminEngine",
    "SouthKoreaAdminEngine",
    "MalaysiaAdminEngine",
    "MexicoAdminEngine",
    "NetherlandsAdminEngine",
    "NorwayAdminEngine",
    "NewZealandAdminEngine",
    "PhilippinesAdminEngine",
    "PortugalAdminEngine",
    "SwedenAdminEngine",
    "SingaporeAdminEngine",
    "SwitzerlandAdminEngine",
    "ThailandAdminEngine",
    "TaiwanAdminEngine",
]
