...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "estonia"

EE_PROFILE = AdminProfile(
    name_keys=("name:et", "name", "name:en", "official_name"),
    level_policies={
        6: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("de", "en", "et", "lt", "ru"),
)
...
class EstoniaAdminEngine(GermanyAdminEngine):
    ENGINE = "ee_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "estonia_admin.json"
        self._ffsf_dataset_path = self._work_dir / "estonia_admin.bin"
        self._ffsf_meta_path = self._work_dir / "EE_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "estonia_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "EstoniaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="ee-admin-semantic-1.0.0",
