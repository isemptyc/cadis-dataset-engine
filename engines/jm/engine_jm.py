...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "jamaica"

JM_PROFILE = AdminProfile(
    name_keys=("name", "official_name", "name:en"),
    level_policies={
        5: AdminLevelPolicy(simplify=True, simplify_tolerance=0.004, fix_invalid=True, parent_resolution="strict"),
    ...
    multilingual_allowed_languages=("en",),
)
...
class JamaicaAdminEngine(BrazilAdminEngine):
    ENGINE = "jm_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [5, 6, 8, 9]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((5, 6, 8, 9))
    ...
    def __init__(self, *, osm_pbf_path: str | Path | None = None, work_dir: Path | None = None, country_geometry_path: str | Path | None = None):
        ...
        self._admin_dataset_path = self._work_dir / "jamaica_admin.json"
        self._ffsf_dataset_path = self._work_dir / "jamaica_admin.bin"
        self._ffsf_meta_path = self._work_dir / "JM_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "jamaica_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError("JamaicaAdminEngine in cadis-dataset-engine is build-only. Provide osm_pbf_path.")
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            export_admin_semantic_dataset(nodes=self._build_semantic_nodes(), output_path=self._semantic_dataset_path, version="jm-admin-semantic-1.0.0", country=self.COUNTRY_ISO, source="admin_tree.txt")
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "allowed_levels": [5, 6, 8, 9],
            "allowed_shapes": allowed_shapes,
            "shape_status": [{"levels": shape, "status": "ok" if 5 in shape or 6 in shape else "partial"} for shape in allowed_shapes],
            "layers": {"hierarchy_required": True, "repair_required": False},
            "hierarchy_repair_rules": {"parent_level": 5, "child_levels": [6, 8, 9]},
