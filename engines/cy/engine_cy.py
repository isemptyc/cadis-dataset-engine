...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "cyprus"

CY_PROFILE = AdminProfile(
    name_keys=("name:el", "name:tr", "name", "name:en", "official_name"),
    level_policies={
        5: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("el", "en", "tr"),
)
...
class CyprusAdminEngine(GermanyAdminEngine):
    ENGINE = "cy_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "cyprus_admin.json"
        self._ffsf_dataset_path = self._work_dir / "cyprus_admin.bin"
        self._ffsf_meta_path = self._work_dir / "CY_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "cyprus_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "CyprusAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="cy-admin-semantic-1.0.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "shape_status": [
                {
                    "levels": shape,
                    "status": "ok" if 5 in shape else "partial",
                }
