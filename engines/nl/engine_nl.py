...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "netherlands"

NL_PROFILE = AdminProfile(
    name_keys=("name:nl", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("nl", "fy", "en"),
)


class NetherlandsAdminEngine(DatasetBuildEngineBase):
    ENGINE = "nl_admin"
    VERSION = "v1.0"
    ...
    EXCLUDED_FEATURE_IDS = {
        "nl_r53137",  # Baarle-Hertog (Belgian enclave inside the Netherlands extract)
    }
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "netherlands_admin.json"
        self._ffsf_dataset_path = self._work_dir / "netherlands_admin.bin"
        self._ffsf_meta_path = self._work_dir / "NL_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "netherlands_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "NetherlandsAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="nl-admin-semantic-1.0.0",
    ...
    def _prune_excluded_features(self) -> None:
        if not self.EXCLUDED_FEATURE_IDS or not self._admin_dataset_path.exists():
            return
        ...
        if isinstance(admin_by_level, dict):
            for key, value in list(admin_by_level.items()):
                if isinstance(value, list):
                    admin_by_level[key] = _filter_rows(value)

        for key, value in list(payload.items()):
            if isinstance(value, list):
                payload[key] = _filter_rows(value)
