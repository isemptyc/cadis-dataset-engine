...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "oman"

OM_PROFILE = AdminProfile(
    name_keys=("name:en", "name", "official_name", "name:om", "name:ru"),
    level_policies={
    ...
    multilingual_allowed_languages=('en', 'om', 'ru'),
)
...
class OmanAdminEngine(BrazilAdminEngine):
    ENGINE = "om_admin"
    VERSION = "v1.0"
    ...
    def __init__(self, *, osm_pbf_path: str | Path | None = None, work_dir: Path | None = None, country_geometry_path: str | Path | None = None):
        ...
        self._admin_dataset_path = self._work_dir / "oman_admin.json"
        self._ffsf_dataset_path = self._work_dir / "oman_admin.bin"
        self._ffsf_meta_path = self._work_dir / "OM_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "oman_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError("OmanAdminEngine in cadis-dataset-engine is build-only. Provide osm_pbf_path.")
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            export_admin_semantic_dataset(nodes=self._build_semantic_nodes(), output_path=self._semantic_dataset_path, version="om-admin-semantic-1.0.0", country=self.COUNTRY_ISO, source="admin_tree.txt")
