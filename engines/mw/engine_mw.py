...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "malawi"

MW_PROFILE = AdminProfile(
    name_keys=("name:en", "name", "official_name", "name:mw", "name:ru"),
    level_policies={
    ...
    multilingual_allowed_languages=('en', 'mw', 'ru'),
)
...
class MalawiAdminEngine(BrazilAdminEngine):
    ENGINE = "mw_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [2, 3, 4, 5, 10]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((2, 3, 4, 5, 10,))
    ...
    def __init__(self, *, osm_pbf_path: str | Path | None = None, work_dir: Path | None = None, country_geometry_path: str | Path | None = None):
        ...
        self._admin_dataset_path = self._work_dir / "malawi_admin.json"
        self._ffsf_dataset_path = self._work_dir / "malawi_admin.bin"
        self._ffsf_meta_path = self._work_dir / "MW_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "malawi_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError("MalawiAdminEngine in cadis-dataset-engine is build-only. Provide osm_pbf_path.")
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            export_admin_semantic_dataset(nodes=self._build_semantic_nodes(), output_path=self._semantic_dataset_path, version="mw-admin-semantic-1.0.0", country=self.COUNTRY_ISO, source="admin_tree.txt")
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "hierarchy_repair_rules": {"parent_level": 2, "child_levels": [level for level in [2, 3, 4, 5, 10] if level != 2]},
