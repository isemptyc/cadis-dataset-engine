...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "luxembourg"

LU_PROFILE = AdminProfile(
    name_keys=("name", "name:lb", "name:fr", "name:de", "name:en", "official_name"),
    level_policies={
        6: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("de", "fr", "lb", "lt", "ru"),
)


class LuxembourgAdminEngine(GermanyAdminEngine):
    ENGINE = "lu_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "luxembourg_admin.json"
        self._ffsf_dataset_path = self._work_dir / "luxembourg_admin.bin"
        self._ffsf_meta_path = self._work_dir / "LU_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "luxembourg_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "LuxembourgAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="lu-admin-semantic-1.0.0",
    ...
    def _runtime_policy_payload(self) -> dict:
        return {
            ...
            "shape_status": [
                ...
                {"levels": [6, 8], "status": "ok"},
                {"levels": [6, 8, 9], "status": "ok"},
