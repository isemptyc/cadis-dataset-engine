...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "denmark"

DK_PROFILE = AdminProfile(
    name_keys=("name:da", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
...
class DenmarkAdminEngine(DatasetBuildEngineBase):
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "denmark_admin.json"
        self._ffsf_dataset_path = self._work_dir / "denmark_admin.bin"
        self._ffsf_meta_path = self._work_dir / "DK_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "denmark_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "DenmarkAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="dk-admin-semantic-1.0.0",
