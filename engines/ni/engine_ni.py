...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "nicaragua"

NI_PROFILE = AdminProfile(
    ...
    level_policies={
        ...
        6: AdminLevelPolicy(simplify=True, simplify_tolerance=0.002, fix_invalid=True, parent_resolution="strict"),
        8: AdminLevelPolicy(simplify=True, simplify_tolerance=0.001, fix_invalid=True, parent_resolution="strict"),
        9: AdminLevelPolicy(simplify=False, simplify_tolerance=None, fix_invalid=False, parent_resolution="strict"),
        10: AdminLevelPolicy(simplify=False, simplify_tolerance=None, fix_invalid=False, parent_resolution="strict"),
        11: AdminLevelPolicy(simplify=False, simplify_tolerance=None, fix_invalid=False, parent_resolution="strict"),
        12: AdminLevelPolicy(simplify=False, simplify_tolerance=None, fix_invalid=False, parent_resolution="strict"),
    },
...
class NicaraguaAdminEngine(BrazilAdminEngine):
    ENGINE = "ni_admin"
    VERSION = "v1.0"
    NAME_SCHEMA = "multilingual_v1"

    LEVELS = [4, 6, 8, 9, 10, 11, 12]
    ALLOWED_SHAPES = _all_nonempty_level_shapes((4, 6, 8, 9, 10, 11, 12))
    ...
    def __init__(self, *, osm_pbf_path: str | Path | None = None, work_dir: Path | None = None, country_geometry_path: str | Path | None = None):
        ...
        self._admin_dataset_path = self._work_dir / "nicaragua_admin.json"
        self._ffsf_dataset_path = self._work_dir / "nicaragua_admin.bin"
        self._ffsf_meta_path = self._work_dir / "NI_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "nicaragua_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError("NicaraguaAdminEngine in cadis-dataset-engine is build-only. Provide osm_pbf_path.")
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        if not self._admin_dataset_path.exists():
            build_admin_dataset(
                ...
                level_labels={
                    ...
                    8: "admin_district",
                    9: "admin_neighborhood",
                    10: "admin_detail",
                    11: "admin_micro_detail",
                    12: "admin_sub_detail",
                },
                id_prefix="ni",
        ...
        if not self._semantic_dataset_path.exists():
            export_admin_semantic_dataset(nodes=self._build_semantic_nodes(), output_path=self._semantic_dataset_path, version="ni-admin-semantic-1.0.0", country=self.COUNTRY_ISO, source="admin_tree.txt")
    ...
    def _runtime_policy_payload(self) -> dict:
        ...
        return {
            ...
            "allowed_levels": [4, 6, 8, 9, 10, 11, 12],
            ...
            "hierarchy_repair_rules": {"parent_level": 4, "child_levels": [6, 8, 9, 10, 11, 12]},
