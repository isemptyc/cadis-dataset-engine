...
DEFAULT_WORK_DIR = Path.home() / ".cache" / "cadis_dataset_engine" / "moldova"

MD_PROFILE = AdminProfile(
    name_keys=("name:ro", "name:ru", "name", "name:en", "official_name"),
    level_policies={
        4: AdminLevelPolicy(
            simplify=True,
    ...
    multilingual_allowed_languages=("en", "ro", "ru", "uk"),
)
...
class MoldovaAdminEngine(GermanyAdminEngine):
    ENGINE = "md_admin"
    VERSION = "v1.0"
    ...
    def __init__(
        ...
        *,
        ...
        self._admin_dataset_path = self._work_dir / "moldova_admin.json"
        self._ffsf_dataset_path = self._work_dir / "moldova_admin.bin"
        self._ffsf_meta_path = self._work_dir / "MD_feature_meta_by_index.json"
        self._semantic_dataset_path = self._work_dir / "moldova_admin_semantic.json"
        ...
        if osm_pbf_path is None:
            raise ValueError(
                "MoldovaAdminEngine in cadis-dataset-engine is build-only. "
    ...
    def _ensure_datasets(self, osm_pbf_path: str) -> None:
        ...
        if not self._semantic_dataset_path.exists():
            ...
            export_admin_semantic_dataset(
                ...
                version="md-admin-semantic-1.0.0",
